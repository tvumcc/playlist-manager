...
def acquire_new_access_token() -> str:
    ...
    try:
        with open("client_info.txt", "r") as client_info_file:
            (clientID, clientSecret) = tuple(client_info_file.readlines())
    except:
        (clientID, clientSecret) = ask_for_client_credentials()
    ...
    if access_token_response.ok:
        ...
        print("ERROR: Your client ID or secret is invalid or incorrect")


"""Asks for the user's client ID and secret to construct a new client_info.txt"""
def ask_for_client_credentials() -> tuple[str, str]:
    print("client_info.txt was not found, enter your client ID and client secret to create it")
    clientID =     input("Client ID: ")
    clientSecret = input("Client Secret: ")

    with open("client_info.txt", "w") as client_info_file:
        client_info_file.write(clientID + "\n" + clientSecret)
...
def get_access_token() -> str:
    ...
    while not validate_token(token): token = acquire_new_access_token()
